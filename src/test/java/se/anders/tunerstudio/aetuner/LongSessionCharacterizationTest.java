...
public final class LongSessionCharacterizationTest {
    ...
    private static void characterizeScaling() throws Exception {
        ...
        System.out.println("LONG_SESSION eventCount,retainedSamples,scalarSlots,sessionReviewMs,"
                + "overviewColdMs,overviewCachedMs,guidanceMs,reportMs,csvMs,csvBytes,resetMs");
        for (int count : EVENT_COUNTS) {
            List<EventSummary> events = events(count);
            ...
            double overviewColdMs = timedInvokeOnEdt(panel, "refreshOverview");
            Object cachedReview = field(panel, "cachedEventReview");
            long cachedRevision = ((Long) field(panel, "cachedReviewEventRevision")).longValue();
            double overviewCachedMs = timedInvokeOnEdt(panel, "refreshOverview");
            require(field(panel, "cachedEventReview") == cachedReview,
                    "Unchanged event revision must reuse the cached SessionReview");
            require(((Long) field(panel, "cachedReviewEventRevision")).longValue() == cachedRevision,
                    "Cached review revision changed without a new event");

            started = System.nanoTime();
            String guidance = (String) invoke(panel, "buildSessionGuidanceText");
    ...
    private static void assertBoundedState() throws Exception {
        ...
        SessionReview emptyReview = SessionReview.build(new ArrayList<EventSummary>(),
                new SessionMonitor().snapshot());
        for (int i = 0; i < 150; i++) {
            history.observe("characterization state " + i, emptyReview,
                    new ArrayList<EventSummary>(), new SessionMonitor().snapshot(),
    ...
    private static void assertResetCleanupAndReuse() throws Exception {
        List<EventSummary> retained = events(1000);
        AeTunerPanel panel = panelWithEvents(projectSnapshot(), retained);
        Object plot = field(panel, "plotPanel");
        Method setEvent = plot.getClass().getDeclaredMethod("setEvent", EventSummary.class);
        ...
        EventSummary reusable = detectorEvent(detector);
    ...
    private static AeTunerPanel panelWithEvents(AeProjectSnapshot project,
                                                List<EventSummary> source) throws Exception {
        ...
        @SuppressWarnings("unchecked")
        List<EventSummary> retained = (List<EventSummary>) field(panel, "capturedEvents");
        retained.addAll(source);
        int accepted = 0;
        for (EventSummary event : source) if (event.isAccepted()) accepted++;
    ...
    private static List<EventSummary> events(int count) {
        List<EventSummary> events = new ArrayList<EventSummary>(count);
        for (int i = 0; i < count; i++) {
            ...
            events.add(new EventSummary(i + 1, accepted,
    ...
    private static EventSummary detectorEvent(AeEventDetector detector) {

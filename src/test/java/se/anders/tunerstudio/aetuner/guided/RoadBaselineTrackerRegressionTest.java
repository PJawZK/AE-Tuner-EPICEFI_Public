...
public final class RoadBaselineTrackerRegressionTest {
    ...
    private static void gradualRoadTrendProducesProjectedBaseline() {
        ...
        for (int i = 0; i < 22; i++) {
            double t = i * 0.05;
            last = sample(t, 1940.0 + i * 5.0,
                    50.0 + i * 0.08, 8.0 + i * 0.02,
                    false, false);
            tracker.add(last);
        }
        RoadBaselineTracker.AcquireCheck check =
                tracker.acquireCheck(last, 2000.0, 0L, 1.50);
        require(check.ready, "gradual road trend did not reach acquire-ready");
    ...
    private static void recoveryIntervalRemainsPartOfAcquireGate() {
        ...
        LiveSample sample = sample(2.0, 2000.0, 51.0, 8.2, false, false);
        tracker.add(sample);
        RoadBaselineTracker.AcquireCheck blocked =
                tracker.acquireCheck(sample, 2000.0,
    ...
    private static void abruptReadyMovementCancelsOnceItEntersHistory() {
        RoadBaselineTracker tracker = settled();
        LiveSample abrupt = sample(1.10, 2000.0, 62.0, 16.0, false, false);
        tracker.add(abrupt);
        RoadBaselineTracker.ReadyCheck immediate =
                tracker.readyCheck(abrupt, 2000.0);
        require(immediate.ready,
                "READY no longer excludes the newest sample from its trend window");

        LiveSample next = sample(1.15, 2000.0, 62.0, 16.0, false, false);
        tracker.add(next);
        RoadBaselineTracker.ReadyCheck check = tracker.readyCheck(next, 2000.0);
        require(!check.ready,
    ...
    private static RoadBaselineTracker settled() {
        ...
        for (int i = 0; i < 22; i++) {
            double t = i * 0.05;
            tracker.add(sample(t, 1980.0 + i * 2.0,

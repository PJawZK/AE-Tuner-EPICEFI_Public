...
public final class MapBlendSuggestionRegressionTest {
    ...
    public static void main(String[] args) {
        ...
        upperBoundDurationsRemainMeasuredAndWithheld();
        lowerBoundDurationsRemainMeasuredBeforeProposalClamp();
        proposalMarginDoesNotAlterDisplayedStatistics();
    ...
    private static void upperBoundDurationsRemainMeasuredAndWithheld() {
        List<TransientEvent> events = new ArrayList<TransientEvent>();
        events.add(event(1, 2000.0, 0.80, false));
        events.add(event(2, 2000.0, 0.95, false));
        events.add(event(3, 2000.0, 1.10, false));
        events.add(event(4, 2000.0, 1.25, false));
        events.add(event(5, 2000.0, 1.40, false));
        MapBlendSuggestion suggestion = MapBlendSuggestion.build(snapshot(), events);
        String report = suggestion.getDisplayText();
        require(!suggestion.isAvailable(), "Upper-bound collapse created a paste-ready proposal");
        require(report.contains("median 1.10 s") && report.contains("range 0.80-1.40 s"),
                "Upper-bound measured spread was not retained");
        require(!report.contains("range 0.80-0.80 s"),
                "Upper-bound measured durations collapsed to the table maximum");
        require(report.contains("Confidence: LOW | eligibility: WITHHELD"),
                "Upper-bound measured distribution did not control confidence and eligibility");
        require(report.contains("withheld because catch-up range is too wide"),
                "Upper-bound spread rejection was not explained");
        require(suggestion.getCopyPasteBlock().length() == 0,
                "Upper-bound withheld evidence exposed a copy block");
        require(report.contains("curve value: 0.30 -> 0.30 s"),
                "Upper-bound withheld point did not remain unchanged");
    }

    private static void lowerBoundDurationsRemainMeasuredBeforeProposalClamp() {
        List<TransientEvent> events = new ArrayList<TransientEvent>();
        events.add(event(1, 1500.0, 0.01, false));
        events.add(event(2, 1500.0, 0.02, false));
        events.add(event(3, 1500.0, 0.03, false));
        events.add(event(4, 1500.0, 0.04, false));
        events.add(event(5, 1500.0, 0.05, false));
        MapBlendSuggestion suggestion = MapBlendSuggestion.build(snapshot(), events);
        String report = suggestion.getDisplayText();
        require(suggestion.isAvailable(), "Tight lower-bound evidence was not eligible");
        require(report.contains("median 0.03 s") && report.contains("range 0.01-0.05 s"),
                "Lower-bound measured statistics were not retained");
        require(!report.contains("range 0.08-0.08 s"),
                "Lower-bound measured durations collapsed to the table minimum");
        require(report.contains("Confidence: HIGH | eligibility: ELIGIBLE"),
                "Lower-bound measured distribution did not control confidence and eligibility");
        String[] values = suggestion.getCopyPasteBlock().split("\\n");
        require("0.30".equals(values[0]), "Unsupported 1000 RPM point changed");
        require("0.08".equals(values[1]), "Final lower-bound proposal was not clamped to 0.08 s");
        require("0.30".equals(values[2]), "Unsupported 2000 RPM point changed");
        require("0.30".equals(values[3]), "Unsupported 2500 RPM point changed");
    }

    private static void proposalMarginDoesNotAlterDisplayedStatistics() {
        List<TransientEvent> events = new ArrayList<TransientEvent>();
        events.add(event(1, 2000.0, 0.20, false));
        events.add(event(2, 2000.0, 0.21, false));
        events.add(event(3, 2000.0, 0.22, false));
        events.add(event(4, 2000.0, 0.23, false));
        events.add(event(5, 2000.0, 0.24, false));
        MapBlendSuggestion suggestion = MapBlendSuggestion.build(snapshot(), events);
        require(suggestion.getDisplayText().contains("median 0.22 s"),
                "Displayed median included the proposal margin");
        require("0.24".equals(suggestion.getCopyPasteBlock().split("\\n")[2]),
                "Final proposal was not measured median plus 0.02 s after clamping and rounding");
    }
    ...
    private static TransientEvent event(int index, double rpm, double catchupSeconds, boolean repeatedStab) {
        ...
        if (base + 100000000L < catchNano) {
            samples.add(sample(base + 100000000L, rpm, 20.0, 60.0, 80.0, true, false, 1.0));
        }
        if (repeatedStab) {
            samples.add(sample(base + 160000000L, rpm, 20.0, 62.0, 80.0, true, true, 2.0));
            samples.add(sample(base + 180000000L, rpm, 20.0, 64.0, 80.0, true, true, 2.0));
        }
        ...
        return new TransientEvent(index, true, "MAP Predict event", "synthetic", samples, true);
    }

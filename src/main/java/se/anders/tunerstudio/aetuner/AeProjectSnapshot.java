...
final class AeProjectSnapshot {
    ...
    AeProjectSnapshot(String configurationName,
                      double[] cycleBins,
                      double[] tpsToBins,
                      double[][] cycleValues,
    ...
    double[] getCycleBins() { return cloneArray(cycleBins); }
    double[] getTpsToBins() { return cloneArray(tpsToBins); }
    double[][] getCycleValues() { return cloneTable(cycleValues); }
    double[] getMapEstimateRpmBins() { return cloneArray(mapEstimateRpmBins); }
    double[] getMapEstimateTpsBins() { return cloneArray(mapEstimateTpsBins); }
    double[][] getMapEstimateTable() { return cloneTable(mapEstimateTable); }
    double[] getBlendDurationRpmBins() { return cloneArray(blendDurationRpmBins); }
    double[] getBlendDurationValues() { return cloneArray(blendDurationValues); }

    boolean isTpsAeEnabled() { return tpsAeEnabled; }
    boolean isWallWettingEnabled() { return wallWettingEnabled; }
    String getWallWettingModel() { return wallWettingModel; }
    boolean isExtraShotEnabled() { return extraShotEnabled; }
    boolean isMapEstimateEnabled() { return mapEstimateEnabled; }
    boolean isDynamicThresholdEnabled() { return dynamicThresholdEnabled; }
    boolean isDynamicThresholdAverageStatic() { return dynamicThresholdAverageStatic; }

    boolean hasAdvancedWallTables() {
        return wallTauRows > 0 && wallTauCols > 0 && wallBetaRows > 0 && wallBetaCols > 0;
    }

    boolean hasMapEstimateTable() {
        return mapEstimateRpmBins.length > 0 && mapEstimateTpsBins.length > 0 && mapEstimateTable.length > 0;
    }

    boolean hasBlendDurationCurve() {
        return blendDurationRpmBins.length > 0 && blendDurationValues.length == blendDurationRpmBins.length;
    }

    boolean isMapPredictWorkflow() {
        return mapEstimateEnabled && !tpsAeEnabled;
    }

    double recommendThresholdForRpm(double rpm) {
        return interpolate(thresholdRpmBins, thresholdValues, rpm, 1.5);
    }

    String expectedSessionModeText() {

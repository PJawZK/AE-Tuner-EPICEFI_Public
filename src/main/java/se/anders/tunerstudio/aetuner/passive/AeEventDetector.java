...
final class AeEventDetector {
    ...
    synchronized TransientEvent addSample(LiveSample sample, double startThreshold) {
        return addSample(sample, startThreshold, false);
    }

    synchronized TransientEvent addSample(LiveSample sample, double startThreshold, boolean mapPredictWorkflow) {
        appendRing(sample);
        ...
        if (inEvent) {
            ...
            if (newCleanOpening && active.size() >= 4) {
                TransientEvent previous = buildSummary(trimToRecent(active, MAX_CAPTURE_SECONDS),
        ...
        if (inEvent) {
            ...
            if (timeout || quietDone) {
                ...
                TransientEvent summary = buildSummary(trimToRecent(active, MAX_CAPTURE_SECONDS), closeNote, timeout, mapPredictWorkflow);
                active.clear();
                inEvent = false;
                eventStartNano = 0L;
                lastActivityNano = 0L;
    ...
    private TransientEvent buildSummary(List<LiveSample> samples, String closeNote, boolean closedByTimeout,
        ...
        if (rejection.length() > 0) {
            return new TransientEvent(nextEventIndex++, false, "Rejected", rejection + noteSuffix(closeNote), samples);
        }
        ...
        if (mapPredictWorkflow) {
            if (stats.mapPredictionSeen) {
                ...
                return new TransientEvent(nextEventIndex++, true, eventClass,
            ...
            if (stats.triggerNearMiss) {
                return new TransientEvent(nextEventIndex++, true, "MAP Predict trigger near miss",
            ...
            return new TransientEvent(nextEventIndex++, false, "Pedal movement only",
        ...
        if (stats.tpsAeFuelProved) {
            return new TransientEvent(nextEventIndex++, true, "TPS AE fuel proved",
        ...
        if (stats.tpsAeStateSeen) {
            return new TransientEvent(nextEventIndex++, true, "TPS AE state only",
        ...
        if (stats.otherTransientPathSeen) {
            return new TransientEvent(nextEventIndex++, true, "Other transient path",
                    "No TPS cycle-AE fuel was visible; MAP Predict, Wall Wetting, or Instant Fuel activity was present"
                            + noteSuffix(closeNote), samples, false);
        }
        if (stats.triggerNearMiss) {
            return new TransientEvent(nextEventIndex++, true, "Trigger near miss",
                    "Pedal movement reached " + formatPercent(stats.maxTriggerRatio)

...
/** Production host for the current v0.19 Guided UI. */
public final class GuidedProductionWorkspacePanel extends JPanel {
    ...
    /** The validated v0.19 workspace is now the only production UI. */
    public static boolean featureEnabled() { return true; }

    public GuidedProductionWorkspacePanel(GuidedCapturePanel productionPanel) {
        this(productionPanel, GuidedV019UtilityActions.NONE);
    }
    ...
    public GuidedProductionWorkspacePanel(GuidedCapturePanel productionPanel,
        ...
        GuidedDialogLifecycle.registerRoot(v019);
        add(v019, BorderLayout.CENTER);
    }

    public void setCurrentTune(AeProjectSnapshot snapshot) { v019.setCurrentTune(snapshot); }

    /** Plugin hide/final close owns all short-lived Guided dialogs. */
    public void disposeOwnedDialogs() { GuidedDialogLifecycle.disposeAll(); }
    ...
    JButton exportSessionButtonForTest() { return v019.exportSessionButtonForTest(); }
    ...
    JComponent tpsFocusViewForTest() { return focusViewForTest(GuidedProductionTask.TPS_MOVEMENT_TIMING); }
    ...
    private void selectTask(GuidedProductionTask task) { v019.selectTaskForTest(task); }
}
